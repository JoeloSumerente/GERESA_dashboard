...
read_data_dpto <- function() {
  data_dpto <- fread("https://raw.githubusercontent.com/gorecuscogrds/dashboard-covid-geresa/main/data/maindata/data_regional.csv", keepLeadingZeros = TRUE)
...
read_data_prov <- function() {
  data_prov <- fread("https://raw.githubusercontent.com/gorecuscogrds/dashboard-covid-geresa/main/data/maindata/data_provincial.csv", keepLeadingZeros = TRUE)
...
read_data_dis <- function() {
  data_dis <- fread("https://raw.githubusercontent.com/gorecuscogrds/dashboard-covid-geresa/main/data/maindata/data_distrital.csv", keepLeadingZeros = TRUE)
...
read_data_map_district <- function() {
  cusco_map_district <- jsonlite::fromJSON("https://raw.githubusercontent.com/gorecuscogrds/dashboard-covid-geresa/main/data/mapas/districts.geojson", simplifyVector = FALSE)
}
